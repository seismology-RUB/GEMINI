...
!
	...
	allocate(ats%y(ns))
	ats%nsamp = ns; ats%dt = dtnew; ats%tanf = tbeg; ats%link = .false.
!
...
!
	allocate(cts%y(nc))
	cts%dt = this%dt; cts%tanf = this%tanf; cts%nsamp = nc; cts%link = .false.
!
...
!
	...
	nc = kmax-kmin+1
	allocate(croco%y(nc))
	croco%nsamp = nc; croco%tanf = kmin*this%dt; croco%dt = this%dt; croco%link = .false.
!
	do k=kmin,-1
		help=0.
...
!
	n = this%nsamp
	allocate(dts%y(n))
	dts%nsamp = n; dts%tanf = this%tanf; dts%dt = this%dt; dts%link = .false.
	do i=2,n-1
...
!
	...
	allocate(env%y(n1))
	env%nsamp = n1; env%tanf = this%tanf; env%dt = this%dt; env%link = .false.
	do i=1,n1
...
!
	...
	integer :: i
!
	allocate(its%y(this%nsamp))
	its%nsamp = this%nsamp; its%tanf = this%tanf; its%dt = this%dt; its%link = .false.
	its%y(1) = 0.
	do i=2,this%nsamp
...
!
	...
	lcts%nsamp = this%nsamp
	lcts%dt = this%dt
	lcts%tanf = this%tanf
	lcts%link = .false.
	allocate(lcts%y(this%nsamp))
...
!
	...
	prts%nsamp = this%nsamp
	prts%dt = this%dt
	prts%tanf = this%tanf
	prts%link = .false.
	allocate(prts%y(this%nsamp))
...
!
	n = this%nsamp
	allocate(fts%y(n))
	fts%nsamp = n; fts%tanf = this%tanf; fts%dt = this%dt; fts%link = .false.
...
!
	...
	nf = (nd-1)/ndec+1
	allocate(rts%y(nf))
	rts%nsamp = nf; rts%tanf = this%tanf; rts%dt = this%dt*ndec; rts%link = .false.
!
...
!> \param wn length of signal window in seconds
!
...
!
	...
	allocate(tts%y(nsamp))
	tts%nsamp = nsamp; tts%tanf = this%tanf; tts%dt = this%dt; tts%link = .false.
