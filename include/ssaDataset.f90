...
!
	double precision function getSourceTimeSSADatablock(this,tref) result(tanf)
	type (ssa_datablock), intent(in) :: this
	integer, intent(in) :: tref
...
!
	integer function getTfsSSADatablock(this) result(tfs)
	type (ssa_datablock), intent(in) :: this
	integer :: tfs
	tfs = this%tfs
...
!
	integer function getTnsSSADatablock(this) result(tns)
	type (ssa_datablock), intent(in) :: this
	integer :: tns
	tns = this%tns
...
!
	...
	if (ndbl == 0) then
		print *,'no datablocks found in ',trim(group%name),group%tag
		stop
	endif
	allocate(this%datablock(ndbl))
	do j=1,ndbl
