...
!
 module sourceReceiver
	...
	interface new
		...
		module procedure sar_CreateSingleStationZRT
	end interface
	interface dealloc
...
! 
	ascii_dez=ichar(adjustl(line(1:1)))
...
!----------------------------------------------------------------------------------------
!> \brief Create a special 3 component, single station info object
!! \param this info object (out)
!! \param event seismic event (in)
!! \param station seismic station (in)
!
	subroutine sar_CreateSingleStationZRT(this,event,station)
	type (sar) :: this
	type (seismic_event) :: event
	type (seismic_station) :: station
	character (len=1), dimension(3) :: comps = (/'Z','R','T'/)
	integer :: j
	real :: sd
!
	call sar_CreateBasic(this,3)
!
!  fill up event information
!
	call sar_setSourceType(this,.styp.event)
	call sar_setCoordinateSystem(this,.csys.event)
	call sar_setEventId(this,.evid.event)
	call sar_setOriginDateTime(this,.timestring.(.otime.event))
	if (.csys.event == 'S') then
		sd = (.sdepth.event)*1.e3
	else
		sd = .sdepth.event
	endif
	call sar_setSourceCoordinates(this,.slat.event,.slon.event,sd)
!
!  add force or moment
!
	if (.styp.event == 0) then
		call sar_setForce(this,.force.event)
	else if (.styp.event == 1) then
		call sar_setMomentTensor(this,.momten.event)
	endif
!
!  trace information in order ZRT
!
	do j = 1,3
		call sar_setNameCompNet(this,j,.staname.station,comps(j),.netcode.station)
		call sar_setStationCoordinates(this,j,.lat.station,.lon.station,.alt.station)
	enddo
	call sar_recomputeEpicentralCoordinates(this)
!
!  Conversion from rad to km for csys = S is not done here !!!!!
!
	end subroutine sar_CreateSingleStationZRT
!----------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------
!> \brief Recompute epicentral coordinates dis and phi
!
	subroutine sar_recomputeEpicentralCoordinates(this)
	type (sar) :: this
	integer :: j
	do j = 1,this%nr
		call sar_epico(this,this%xsta(j),this%ysta(j),this%dis(j),this%phi(j))
	enddo
	end subroutine sar_recomputeEpicentralCoordinates
!----------------------------------------------------------------------
...
!> \param x Latitude of source in degrees or x-coordinate in meters
!> \param y Longitude of source in degrees or y-coordinate in meters
!> \param z Depth of source in meters
!
...
!----------------------------------------------------------------------
!> \brief set source depth
!> \param sd New source depth in meters
!
	subroutine sar_setSourceDepth(this,sd)
	type (sar) :: this
	real :: sd
	if (this%csys == 'S') then
		this%depsrc = sd*1.e-3
	else
		this%depsrc = sd
	endif
	end subroutine sar_setSourceDepth
!----------------------------------------------------------------------
!> \brief set event id
!
...
!> \brief set force
!
	subroutine sar_setForce(this,force)
	type (sar) :: this
	real, dimension(:) :: force
	this%force = force
	this%istyp = 0
	end subroutine sar_setForce
!----------------------------------------------------------------------
!> \brief set momen tensor
!
	subroutine sar_setMomentTensor(this,moment)
	type (sar) :: this
	real, dimension(:) :: moment
	this%mt = moment
	this%istyp = 1
	end subroutine sar_setMomentTensor
!----------------------------------------------------------------------
!> \brief Write info data to file
!
	subroutine sar_writeInfo(this,lu,filename)
	type (sar) :: this
	integer :: lu,j
...
!
	...
	write(lu,'(a13)') this%eventid
	write(lu,'(a1,1x,i1,1x,i1)') this%csys,this%netflag,gg2gcflag
	write(lu,*) this%year,this%month,this%day,this%hour,this%minute,this%sec,this%cendt,hdur
	write(lu,*) this%xsrc,this%ysrc,this%depsrc*1.e3,this%istyp
	if(this%istyp.eq.1) then
		write(lu,*) (this%mt(j),j=1,6)
	else if(this%istyp.eq.0) then
		write(lu,*) (this%force(j),j=1,3)
	endif
	write(lu,'(i5.5)') this%nr
	do j = 1,this%nr
		if (this%csys == 'S') then; dsta = this%depsta(j)*1.e3; else; dsta = this%depsta(j); endif
		write(lu,'(i4,1x,a5,1x,a1,1x,a2,3x,f10.4,1x,f10.4,1x,f5.0)') & 
     &		        j,this%name(j),this%comp(j),this%net(j),this%xsta(j),this%ysta(j),dsta
	enddo
	close(lu)
	end subroutine sar_writeInfo
!----------------------------------------------------------------------
