...
c-----------------------------------------------------------------------------------
	subroutine add_sph_displ(i,f,p,cg,qatt,psi,rearth,ur,vr,rr,zdis)
	include 'sardim.h'
	include 'sar.h'
	real f,p,cg,qatt,rearth,ur,vr,rr,azi,caz,saz
	complex pot,dt_pot,df_pot,zspl,zspt,zspn,zspe
	integer i
	complex psi(0:2),zdis

	if(sar_comp(i).eq.'Z' .or. sar_comp(i).eq.'R') then
		call potential(sar_dis(i),sar_phi(i),f,p,cg,qatt,psi,rearth,pot,1)
		zdis=zdis+ur*pot
	else if(sar_comp(i).eq.'H') then
		call potential(sar_dis(i),sar_phi(i),f,p,cg,qatt,psi,rearth,pot,1)
		zdis=zdis+rr*pot
